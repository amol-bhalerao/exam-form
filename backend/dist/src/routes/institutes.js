...
institutesRouter.post('/register', async (req, res) => {
    ...
    if (existingInstituteAdmin)
        return res.status(409).json({ error: 'INSTITUTE_ADMIN_ALREADY_EXISTS' });
...
institutesRouter.post('/users/invite/:token/complete', async (req, res) => {
    ...
    if (existingUsername && existingUsername.id !== invite.user.id)
        return res.status(409).json({ error: 'USERNAME_TAKEN' });
...
institutesRouter.patch('/users/:id/approve', requireAuth, requireRole(['SUPER_ADMIN']), async (req, res) => {
    ...
    if (!user || user.role.name !== 'INSTITUTE')
...
institutesRouter.get('/board/teachers', requireAuth, requireRole(['BOARD']), async (req, res) => {
    ...
    const where = {};
    if (q.instituteId)
        where.instituteId = q.instituteId;
    if (q.active)
        where.active = q.active === 'true';
    if (q.governmentId)
        where.governmentId = { contains: q.governmentId };
    if (q.teacherType)
        where.teacherType = q.teacherType;
    if (q.certificate)
        where.certifications = { contains: q.certificate };
    if (q.subject)
        where.subjectSpecialization = { contains: q.subject };
    if (q.teacherType)
        where.teacherType = q.teacherType;
    if (q.casteCategory)
        where.casterCategory = q.casteCategory;
    if (q.serviceStartDateFrom || q.serviceStartDateTo) {
        where.serviceStartDate = {};
        if (q.serviceStartDateFrom)
            where.serviceStartDate.gte = new Date(q.serviceStartDateFrom);
        if (q.serviceStartDateTo)
            where.serviceStartDate.lte = new Date(q.serviceStartDateTo);
    }
    if (q.leavingDateFrom || q.leavingDateTo) {
        where.leavingDate = {};
        if (q.leavingDateFrom)
            where.leavingDate.gte = new Date(q.leavingDateFrom);
        if (q.leavingDateTo)
            where.leavingDate.lte = new Date(q.leavingDateTo);
    }
...
institutesRouter.post('/me/teachers', requireAuth, requireRole(['INSTITUTE']), async (req, res) => {
    ...
    if (existingTeacher)
        return res.status(409).json({ error: 'DUPLICATE_TEACHER' });
