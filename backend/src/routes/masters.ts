...
const subjectCategories = ['language', 'Compulsory', 'Optional Subjects', 'Bifocal Subjects', 'Vocational Subjects'] as const;
...
mastersRouter.post('/subjects', requireAuth, requireRole(['SUPER_ADMIN', 'BOARD']), async (req, res) => {
  const body = z
    .object({
      name: z.string().min(2),
      code: z.string().min(1),
      category: z.enum([...subjectCategories] as [string, ...string[]])
    })
    .parse(req.body);
...
mastersRouter.put('/subjects/:id', requireAuth, requireRole(['SUPER_ADMIN', 'BOARD']), async (req, res) => {
  ...
  const body = z
    .object({
      ...
      category: z.enum([...subjectCategories] as [string, ...string[]]).optional()
    })
    .parse(req.body);
