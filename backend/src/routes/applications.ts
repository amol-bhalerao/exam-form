...
async function getApplicationScoped(applicationId: number, auth: { userId: number; role: string; instituteId: number | null }) {
...
applicationsRouter.post('/:id/institute/decision', requireAuth, requireRole(['INSTITUTE']), async (req, res) => {
  ...
  const toStatus: ApplicationStatus = body.action === 'VERIFY' ? 'INSTITUTE_VERIFIED' : 'REJECTED_BY_INSTITUTE';
...
applicationsRouter.post('/:id/board/decision', requireAuth, requireRole(['BOARD']), async (req, res) => {
  ...
  const toStatus: ApplicationStatus = body.action === 'APPROVE' ? 'BOARD_APPROVED' : 'REJECTED_BY_BOARD';
