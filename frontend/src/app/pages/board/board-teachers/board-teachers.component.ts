  ...
  template: `
    <mat-card class="card">
      <div class="dashboard-grid">
        <div class="stat">Total Teachers: {{ metadata().total }}</div>
        <div class="stat">Active: {{ metadata().activeCount }}</div>
        <div class="stat">Inactive: {{ metadata().inactiveCount }}</div>
        <div class="stat">Multi-Institute: {{ metadata().multiInstituteCount }}</div>
      </div>
      <div class="controls">
...
export class BoardTeachersComponent implements OnInit {
  ...
  readonly metadata = signal({ page: 1, limit: 20, total: 0, activeCount: 0, inactiveCount: 0, multiInstituteCount: 0 });
  search = '';
  statusFilter = '';
  instituteFilter = '';
  dutyFilter = '';
  ...
  printGrid() {
    const rows = this.teachers().map((t) => `<tr><td>${t.fullName}</td><td>${this.formatDate(t.dob)}</td><td>${t.governmentId ?? ''}</td><td>${t.subjectSpecialization ?? ''}</td><td>${t.instituteNames ?? t.institute?.name ?? ''}</td><td>${this.formatDate(t.joiningDate)}</td><td>${this.formatDate(t.retirementDate)}</td><td>${t.totalYearsService ?? ''}</td><td>${t.seniorPayGradeEligible ? 'Eligible' : 'No'}</td><td>${t.selectionPayGradeEligible ? 'Eligible' : 'No'}</td><td>${t.examinerExperienceYears ? `${t.examinerExperienceYears} yrs` : 'No'}</td><td>${t.moderatorExperienceYears ? `${t.moderatorExperienceYears} yrs` : 'No'}</td><td>${t.chiefModeratorExperienceYears ? `${t.chiefModeratorExperienceYears} yrs` : 'No'}</td><td>${t.email ?? ''}</td><td>${t.mobile ?? ''}</td><td>${t.active ? 'Active' : 'Inactive'}</td></tr>`).join('');
    const html = `<html><head><style>body{font-family:Arial,sans-serif;padding:16px;} .summary{margin-bottom:12px;font-size:14px;} table{width:100%;border-collapse:collapse;}th,td{border:1px solid #666;padding:6px;text-align:left;} th{background:#eef2ff;}</style></head><body><h2>Board Teacher Summary</h2><div class="summary">Generated: ${new Date().toLocaleString()}<br/>Total: ${this.metadata().total} | Active: ${this.metadata().activeCount} | Multi-Institute: ${this.metadata().multiInstituteCount}</div><table><thead><tr><th>Name</th><th>DOB</th><th>Aadhar</th><th>Subject</th><th>Institutes</th><th>Joining</th><th>Retirement</th><th>Total Service</th><th>Senior Grade</th><th>Selection Grade</th><th>Examiner</th><th>Moderator</th><th>Chief Moderator</th><th>Email</th><th>Mobile</th><th>Status</th></tr></thead><tbody>${rows}</tbody></table></body></html>`;
